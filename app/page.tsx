...
import { getItems, deleteItem, addItem, updateItem } from "../lib/indexeddb";
...
export default function Page() {
  ...
  const [editingItem, setEditingItem] = useState<ItemDataList | null>(null);
...
console.log(dateTime)
  ...
  const handleSave = async () => {
    if (data.trim()) {
      if (editingItem) {
        await updateItem(editingItem.id, { value: data ,dateTime:dateTime });
        setEditingItem(null); 
      } else {
        await addItem({ value: data, dateTime:dateTime });
      }
  ...
  const handleEditItem = (item: ItemDataList) => {
    setEditingItem(item);
    setData(item.value); 
  };
...
setDateTime(value.format("MM/DD/YYYY"))
}
  return (
        ...
        <ul className="w-full">
          {items?.map((item: ItemDataList, index: number) => (
            ...
            >
              ...
              <div>
              <Button className="bg-blue-500 text-white" onClick={() => handleEditItem(item)}>ویرایش</Button>
              <Button
        ...
        <AnimatePresence mode="popLayout">
          {(newTask ||editingItem) && (
            <motion.div
              ...
              className="w-11/12 h-[550px]    bg-blue-100 absolute bottom-0 top-0 my-auto z-20 right-0 left-0 mx-auto rounded-xl"
            >
              <div className=""> 
              ...
              <Input
                ...
                className="w-3/4 h-11 mt-5 mb-4 "
                value={data}
                onChange={(e) => setData(e.target.value)}
              />
              </div>
              <div className="w-full flex gap-10 mx-auto ">
              <label className="" htmlFor="title">
                تاریخ تسک
              </label>
              <DatePicker
                onChange={ChangeDateTime}
                inputClass="w-full h-8 rounded"
              ...
              <motion.div className="flex justify-center gap-4 p-2  absolute bottom-5 right-0 left-0 ">
                <Button
                  ...
                  onClick={handleSave}
                >
                  {editingItem ? "Update" : "ثبت"}
                  </Button>
                <Button
                  className="bg-red-700 text-white w-2/4 font-bold text-[16px]"
                  title="click"
                  onClick={() => {setNewTask(false), setEditingItem(null)}}
                >
                  لغو کردن
                </Button>
              </motion.div>
